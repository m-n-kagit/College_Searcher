...
export const getMySavedItems = async (req: Request, res: Response) => {
  try {
...
export const addToSaved = async (req: Request, res: Response) => {
  try {
    const userId = getUserIdFromReq(req);
    if (!userId) return res.status(401).json({ success: false, message: 'Unauthorized' });

    const collegeId = String(req.params?.collegeId ?? '').trim();
    if (!collegeId) return res.status(400).json({ success: false, message: 'Missing collegeId param' });
...
export const removeFromSaved = async (req: Request, res: Response) => {
  try {
    const userId = getUserIdFromReq(req);
    if (!userId) return res.status(401).json({ success: false, message: 'Unauthorized' });
